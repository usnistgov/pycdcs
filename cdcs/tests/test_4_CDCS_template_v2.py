...
class TestCDCS():
    ...
    @responses.activate
    def test_get_template_managers_v2(self):
        ...
        managers = self.cdcs_v2.get_template_managers()
        assert managers.title.tolist() == ['first', 'second']

        # Test get_template_managers with useronly=True
        managers = self.cdcs_v2.get_template_managers(useronly=True)
        assert len(managers) == 0

        # Test get_template_managers with title
        managers = self.cdcs_v2.get_template_managers(title='first')
        assert managers.title[0] == 'first'

        # Test get_template_managers with is_disabled=True
        managers = self.cdcs_v2.get_template_managers(is_disabled=True)
    ...
    def test_get_template_v2(self):
        ...
        # Test template
        template = self.cdcs_v2.get_template('second', current=True)
        assert template.id == '3'

        # Test template errors for multiple, no matches
        with raises(ValueError):
            template = self.cdcs_v2.get_template()
        with raises(ValueError):
            template = self.cdcs_v2.get_template('zeroth')

    @responses.activate
    def test_template_titles_v2(self):
        ...
        assert self.cdcs_v2.template_titles == ['first', 'second']

    @responses.activate
    def test_empty_template_titles_v2(self):
        """Unique test for initial database"""
        responses.add(responses.GET,
                    f'{self.host}/rest/template-version-manager/global/',
                    status=200, json=[],
                    match=[responses.matchers.query_param_matcher({})])
        assert self.cdcs_v2.template_titles == []

    @responses.activate
    def test_upload_template_v2(self):
        ...
        title = 'test'
        self.cdcs_v2.upload_template(title=title, content=content)
    ...
    def test_update_template_v2(self):
        ...
        title = 'first'
        self.cdcs_v2.update_template(title=title, content=content)
    ...
    @responses.activate
    def test_set_current_template_v2(self):
        """Tests set_current_template()"""
        # Add Mock responses
        template_manager_responses(self.host, 2)
        template_responses(self.host, 2)
        
        self.cdcs_v2.set_current_template('second', version=1)
        
        with raises(ValueError):
            self.cdcs_v2.set_current_template('second', version=2)
        
        with raises(IndexError):
            self.cdcs_v2.set_current_template('first', version=2)